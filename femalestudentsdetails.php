...
<h1 style="width:98%;background:rgba(0,0,0,0.9);color:white;border-radius:25px;margin-top:10px;text-align:center;">Students' Details(FEMALE)</h1>
...
$selectpifemale="select `profileimage` from files where gender='female';";
$selectsdfemale="select * from studentsdetails where gender='female';";
$resultsdfemale=$conn->query($selectsdfemale);
$resultpifemale=$conn->query($selectpifemale);
if($resultsdfemale->num_rows>0){
...
<td colspan="3"><form method="post"><select style="background:white;margin-right:5px;" name="gen"><option>female</option><option>all</option><option>male</option></select><input type="submit" value="Go" name="filter" style="width:30px;"/></form></td>
</tr>
<tr>
...
$sno=1;
while(
$rowsdfemale=$resultsdfemale->fetch_assoc() and $rowpifemale=$resultpifemale->fetch_assoc()){
echo '<tr>
<td>'.$sno.'</td><td>'.$rowsdfemale['enrollmentno'].'</td><td><img src="Files/'.$rowpifemale['profileimage'].'" alt="image not available" height="80" width="70"/></td><td>'.$rowsdfemale['name'].'</td><td>'.$rowsdfemale['branch'].'</td><td>'.$rowsdfemale['sem'].'</td><td>'.$rowsdfemale['shift'].'</td><td>'.$rowsdfemale['year'].'</td><td>'.$rowsdfemale['dob'].'</td><td>'.$rowsdfemale['gender'].'</td><td>'.$rowsdfemale['bloodgrp'].'</td><td>'.$rowsdfemale['mobile'].'</td><td><div style="width:100px;word-wrap:break-word;">'.$rowsdfemale['email'].'</div></td>
</tr>';
$sno++;
  }
echo '</table></div>';
