...
<h1 style="width:98%;background:rgba(0,0,0,0.9);color:white;border-radius:25px;margin-top:10px;text-align:center;">More Details</h1>
...
$select="select * from moredetails";
...
<tr><th colspan="14">Total No of Records are : '.$result->num_rows.'</th></tr>
<tr>
<th>SNo.</th><th>Enrollment No.</th><th>Name</th><th>Jeep Roll No.</th><th>Rank</th><th>UBTER Roll No.</th><th>Area</th><th>Religion</th><th>Mino<br>rity</th><th>Cate<br>gory</th><th>Sub-Cate<br>gory</th><th>City</th><th><div style="width:60px;word-wrap:break-word;">Correspondence Address</div></th><th><div style="width:60px;word-wrap:break-word;">Permanent Address</div></th>
</tr>';
$sno=1;
while($row=$result->fetch_assoc()){
echo '<tr>
<td>'.$sno.'</td><td>'.$row['enrollmentno'].'</td><td>'.$row['name'].'</td><td>'.$row['jeeprno'].'</td><td>'.$row['rank'].'</td><td>'.$row['ubterrno'].'</td><td>'.$row['area'].'</td><td>'.$row['religion'].'</td><td>'.$row['minority'].'</td><td>'.$row['category'].'</td><td>'.$row['subcategory'].'</td><td>'.$row['city'].'</td><td><div style="width:60px;word-wrap:break-word;">'.$row['coraddress'].'</div></td><td><div style="width:60px;word-wrap:break-word;">'.$row['peraddress'].'</div></td>
</tr>';
$sno++;
  }
echo '</table></div>';
